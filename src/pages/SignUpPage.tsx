...
import { UserPlus, Mail, Lock, AlertCircle, Info, User } from 'lucide-react';
      ...
      <div className="max-w-md mx-auto">
        ...
        >
          <div className="text-center mb-6 sm:mb-8">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold mb-2">Create Account</h1>
            <p className="text-gray-400 text-sm sm:text-base">Join SMARTBREW today</p>
          </div>
          ...
          {successMessage && (
            <motion.div 
              initial={{ opacity: 0, y: -10 }} 
              animate={{ opacity: 1, y: 0 }}
              className="mb-6 p-4 bg-blue-500/20 border border-blue-500 rounded-lg flex items-center"
            >
              <Info className="text-blue-500 mr-3" size={20} />
              <p className="text-blue-200 text-sm">{successMessage}</p>
            </motion.div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div>
              <label className="block text-xs sm:text-sm font-medium text-gray-300 mb-1">Confirm Password</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Lock className="h-4 w-4 sm:h-5 sm:w-5 text-gray-500" />
                </div>
              ...
              <div className="mt-2">
                <label className="flex items-center">
                  <input
                    ...
                    className="w-3 h-3 sm:w-4 sm:h-4 text-blue-600 bg-gray-700 border-gray-600 rounded focus:ring-blue-500 focus:ring-2"
                  />
                  <span className="ml-2 text-xs sm:text-sm text-gray-400">Show Confirm Password</span>
                </label>
              </div>
            </div>
            ...
            >
              {loading ? (
                ...
                <>
                  Sign Up <UserPlus className="ml-2 h-4 w-4 text-white" />
                </>
              )}
            </motion.button>
          </form>
